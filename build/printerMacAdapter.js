...
const usbDetect = __importStar(require("usb-detection"));
const escpos_1 = require("escpos");
class PrinterMacAdapter extends printer_1.Adapter {
    constructor() {
        super();
        /**
         * 打印机列表
    ...
    getDevices(refresh = false) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!refresh && Object.keys(this._devices).length)
                return this.devices;
            this._mointoring();
    ...
    _stopMointoring() {
        ...
        setTimeout(() => {
            ...
            if (PrinterMacAdapter.usbDetectMointoring)
                return false;
    ...
    printerDirect(vid, pid, command) {
        return new Promise((resolve, reject) => {
            ...
            // use escpos module
            const device = new escpos_1.USB(vid, pid);
            ...
            device.open(err => {
                ...
                printer.adapter.write(command, err => {
                    ...
                    setTimeout(() => resolve(result), printer_1.Adapter.EXEC_DELAY);
                });
            });
        });
    }
    ...
    _onceOpenCashBox(vid, pid) {
        return new Promise((resolve, reject) => {
            ...
            device.open(err => {
                ...
                setTimeout(resolve, printer_1.Adapter.EXEC_DELAY); // 每次执行后加入延迟 see https://github.com/tessel/node-usb/issues/254
            });
        });
    }
    // /**
    ...
    //         })
    // }
    _getDeviceById(vid, pid) {
        for (const name in this._devices) {
            if (this._devices[name].deviceDescriptor.idProduct === pid && this._devices[name].deviceDescriptor.idVendor === vid)
                return this._devices[name];
        }
        throw new Error('[Printer] device not exists ...');
    }
}
PrinterMacAdapter.usbDetectMointoring = false; // 是否正在获取打印机列表, 在调用 `getDevices()` 时为 `true`, 在 `usbDetect.stopMonitoring()` 时会判断该值
PrinterMacAdapter.GET_DEVICE_TIMEOUT = 10000; // `stopMointoring()` 执行间隔
/**
 * USB Class Codes
 * @docs http://www.usb.org/developers/defined_class
 */
PrinterMacAdapter.IFACE_CLASS = {
    AUDIO: 0x01,
    HID: 0x03,
    PRINTER: 0x07,
    HUB: 0x09
};
exports.default = PrinterMacAdapter;
